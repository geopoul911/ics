...
const CHANGE_ARRIVAL_FLIGHT = "http://localhost:8000/api/groups/change_arrival_flight/";
...
const GET_RAILWAY_STATIONS = "http://localhost:8000/api/view/get_all_railway_stations/";
const GET_TRAIN_TICKET_AGENCIES = "http://localhost:8000/api/view/get_all_train_ticket_agencies/";

function ChangeArrivalFlight(props) {
  const [show, setShow] = useState(false);
  const handleClose = () => setShow(false);
  const handleShow = () => setShow(true);

  const [arrType, setArrType] = useState(props.group.arrival_type);
  ...
  const parseArrivalInfo = (arrival, type) => {
    if (type !== 'AIR' || typeof arrival !== 'string') return { abbr: "", flight: "" };
  ...
  const { abbr: arrivalAbbr, flight: arrivalFlight } = parseArrivalInfo(props.group.arrival, props.group.arrival_type);
  
  const [flightAbbr, setFlightAbbr] = useState(arrivalAbbr);
  const [Flight, setFlight] = useState(arrivalFlight);
  ...
  const updateArrivalFlight = () => {
    axios({
      method: "post",
      url: CHANGE_ARRIVAL_FLIGHT + props.group.refcode,
      headers: headers,
      data: {
        transport_type: arrType,
        arrival_date: ArrDate.toString(),
        airport: Airport,
        flight: Flight,
        terminal: Terminal,
        airline: Airline,
  ...
  useEffect(() => {
      const rawArrival = props.group.arrival || "";
      const arrivalParts = rawArrival.split(" - ");
  
      // Safely parse date
      const rawDate = arrivalParts[0] || "";
      const parsedDate = new Date(rawDate);
      setArrDate(isNaN(parsedDate.getTime()) ? new Date() : parsedDate);
  
      // Parse airline and flight number safely
      const airlineFlightParts = (arrivalParts[1] || "").trim().split(" ");
      ...
      const airportLocation = arrivalParts[2] || "";
      const terminal = arrivalParts[3] || "No Terminal";
  ...
  }, [props.group.arrival, props.group.arrival_type, AllAirlines]);
  
  // Add new useEffect to handle setting airport after airports are loaded
  useEffect(() => {
    if (airportsLoaded && props.group.arrival_type === 'AIR') {
      const arrivalParts = props.group.arrival.split(" - ");
      const airportLocation = arrivalParts[2];
  ...
  }, [airportsLoaded, props.group.arrival, props.group.arrival_type, AllAirports]);
    ...
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Change Arrival for {props.group.refcode}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <ListGroup>
            <ListGroup.Item>
          ...
          {arrType === "AIR" ? (
            <>
              <small className="mr-auto">
                ...
                />
                Arrival flight is formed by date, time, airline's abbreviation,
                flight number,
                <br />
          ...
          {arrType === "TRN" ? (
            <>
              <small className="mr-auto">
                ...
                />
                Arrival is formed by date, time, railway station, country, train
          ...
          {arrType === "NA" ? (
            <>
              <small className="mr-auto">
                ...
                />
                Arrival is formed by date and time
              </small>
              <Button color="red" onClick={handleClose}>
                Close
              </Button>
...
export default ChangeArrivalFlight;

...
const ADD_PRODUCT = "http://localhost:8000/api/view/add_product/";
...
function AddProduct(props) {
  ...
  const createNewEntertainmentProduct = () => {
    axios({
      method: "post",
      url: ADD_PRODUCT,
      headers: headers,
      data: {
        name: Name,
        entertainment_supplier_id: getEntertainmentSupplierId(),
        lat_lng: latlng,
    ...
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title> Add a new product for {props.name} </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          ...
          <Form.Label style={{marginTop: 20}}>Lat / Lng</Form.Label>
          <Form.Control
            type="text"
            maxLength="80"
            style={{ width: 400 }}

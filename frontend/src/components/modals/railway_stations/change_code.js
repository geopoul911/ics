...
const CHANGE_CODE =
  "http://localhost:8000/api/data_management/change_railway_station_code/";
...
function ChangeCode(props) {
  ...
  const update_Name = () => {
    axios({
      ...
      headers: headers,
      data: {
        railway_station_id: props.railway_station_id,
        code: code,
      },
    })
      .then((res) => {
        props.update_state(res.data.railway_station);
      })
      .catch((e) => {
        Swal.fire({
          icon: "error",
  ...
  return (
    <>
      <FiEdit2
        title={"edit Railway Station's Code"}
        id={"edit_port_name"}
        onClick={() => {
          handleShow();
        }}

...
const ADD_DRIVER = "http://localhost:8000/api/view/add_driver/";
      ...
      .then((res) => {
        if (props.redir) {
          window.location.href =
            "/data_management/driver/" + res.data.new_driver_id;
        } else {
          if (selfEmployed) {
            props.set_driver(Name + " - " + Name);
          } else {
            props.set_driver(Name + " - " + CoachOperator.split(") ")[1]);
          }
        }
      })
      .catch((e) => {
      ...
      >
            ...
            <Col>
              <Form>
                ...
                <Form.Group>
                  ...
                  <Form.Label>Coach Operator</Form.Label>
                  <Form.Check
                    name={"self_or_another"}
                    label={"Driver has himself as a coach operator"}
                    onChange={(e) => {
                      setSelfEmployed(true);
                    }}
                    type={"radio"}
                  ...
                  <Form.Check
                    name={"self_or_another"}
                    label={"Driver belongs to another coach operator"}
                    onChange={(e) => {
                      setSelfEmployed(false);
                    }}
                    type={"radio"}
                    style={{ marginTop: 5, marginRight: 10, marginBottom: 20 }}
                  />
                  {!selfEmployed ? (
                    <>
                      <Autocomplete
        ...
        <Modal.Footer>
          <small className="mr-auto">
            ...
            />
            All fields except Tel2 and Tel3 are required to create a driver.
            {Name.length < 2 ||
            !tel ||
            (!selfEmployed && !CoachOperator) ? (
              <>
                <ul
                  className="mr-auto"
                ...
                >
                  ...
                  <li>
                    {!selfEmployed && !CoachOperator ? (
                      <>
                        <AiOutlineWarning style={warningStyle} />
                        Fill The Coach Operator Field or Add Driver As Coach
                        Operator
                      </>
                    ) : (
                      ""

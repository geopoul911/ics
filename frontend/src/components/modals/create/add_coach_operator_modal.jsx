...
const ADD_COACH_OPERATOR = "http://localhost:8000/api/view/add_coach_operator/";
...
function AddCoachOperatorModal(props) {
      ...
      .then((res) => {
        if (props.redir) {
          window.location.href =
            "/data_management/coach_operator/" + res.data.new_coach_operator_id;
        } else {
          props.set_coach_operator(name);
        }
      })
      .catch((e) => {
        Swal.fire({
    ...
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title> Create new Coach Operator </Modal.Title>
        </Modal.Header>
        <Modal.Body>
            ...
            <Col>
              <Form>
                <Form.Group>
                  ...
                  />
                  <hr/>
                  <label> Coach Operator Categories :</label>
                  <br/>
                  {AllCoachOperatorCategories.map((cc) => (
                    <React.Fragment key={cc} style={{ display: "inline" }}>
                      <Form.Check
                        label={cc.name}
                        checked={checkedCategories.includes(cc)}
                        onChange={() => handleCategoryChange(cc)}
                        style={{
                          display: "inline-block",
        ...
        <Modal.Footer>
          <small className="mr-auto">
            ...
            />
            All fields except Tel2 and Tel3 are required to create a Coach
            Operator.
            {name.length < 2 ||
            Address.length < 5 ||
            !tel ||
                ...
                >
                  ...
                  <li>
                    {checkedCategories.length === 0 ? (
                      <>
                        <AiOutlineWarning style={warningStyle} /> Add at least one Category.
                      </>
                    ) : (
                      ""
                    )}
...
export default AddCoachOperatorModal;

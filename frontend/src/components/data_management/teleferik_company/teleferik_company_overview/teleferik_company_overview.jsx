...
// Variables
const VIEW_TELEFERIK_COMPANY =
  "http://localhost:8000/api/data_management/teleferik_company/";
...
class TeleferikCompanyOverView extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      teleferik_company: {},
    ...
    axios
      ...
      .then((res) => {
        this.setState({
          ...
          notes: res.data.teleferik_company.notes,
          contact_persons: res.data.teleferik_company.contact_persons,
          is_loaded: true,
        });
      })
      .catch((e) => {
  ...
  update_state = (update_state) => {
    this.setState({ teleferik_company: update_state });
  };
  ...
  update_teleferik_company = (teleferik_company) => {
    this.setState({ teleferik_company: teleferik_company });
  };

  add_contact_person = (contact_persons) => {
    var teleferik_company = { ...this.state.teleferik_company };
    teleferik_company.contact_persons = contact_persons;
    this.setState({
    ...
    return (
      <>
        <div className="mainContainer">
          {pageHeader(
            "teleferik_company_overview",
            this.state.teleferik_company.name
          )}
          {this.state.forbidden ? (
            <>{forbidden("Teleferik company Overview")}</>
                  ...
                  <Grid.Column>
                    <Card>
                      ...
                      <Card.Body>
                        ...
                        <ChangeName
                          object_id={this.state.teleferik_company.id}
                          object_name={this.state.teleferik_company.name}
                          object_type={"Teleferik Company"}
                          update_state={this.update_state}
                        />

                        <div className={"info_descr"}> <IoMdBusiness style={overviewIconStyle} /> Company </div>
                        <div className={"info_span"}>
                          {this.state.teleferik_company.payment_details
                            ? this.state.teleferik_company.payment_details.company === "" ||
                              this.state.teleferik_company.payment_details.company === null
                              ? "N/A"
                              : 
                              <span style={{color: this.state.teleferik_company.payment_details.company === this.state.teleferik_company.name ? 'blue': ''}}>
                                {this.state.teleferik_company.payment_details.company}
                              </span>
                            : "N/A"}
                        </div>
                        ...
                        <div className={"info_span"}>
                          {this.state.teleferik_company.contact.address
                            ? this.state.teleferik_company.contact.address
                            : "N/A"}
                        </div>
                        <ChangeAddress
                        ...
                        {this.state.show_address2 ? (
                          <>
                            ...
                            <div className={"info_span"}>
                              {this.state.teleferik_company.contact.address2
                                ? this.state.teleferik_company.contact.address2
                                : "N/A"}
                            </div>
                            <FaMinus
                            ...
                            <ChangeAddress2
                              ...
                              address={
                                this.state.teleferik_company.contact.address2
                                  ? this.state.teleferik_company.contact
                                      .address2
                                  : "N/A"
                              }
                            />
                        ...
                        <div className={"info_span"}>
                          {this.state.teleferik_company.contact.tel
                            ? this.state.teleferik_company.contact.tel
                            : "N/A"}
                        </div>
                        <ChangeTel
                        ...
                        {this.state.show_tel2 ? (
                          <>
                            ...
                            <div className={"info_span"}>
                              {this.state.teleferik_company.contact.tel2
                                ? this.state.teleferik_company.contact.tel2
                                : "N/A"}
                            </div>
                            ...
                            <div className={"info_span"}>
                              {this.state.teleferik_company.contact.tel3
                                ? this.state.teleferik_company.contact.tel3
                                : "N/A"}
                            </div>
                        ...
                        <div className={"info_span"}>
                          {this.state.teleferik_company.region ? this.state.teleferik_company.region : 'N/A'}
                        </div>
                        ...
                        <div className={"info_span"}>
                          {this.state.teleferik_company.contact.postal
                            ? this.state.teleferik_company.contact.postal
                            : "N/A"}
                        </div>
                        <ChangePostal
                        ...
                        <div className={"info_span"}>
                          {this.state.teleferik_company.contact.email
                            ? this.state.teleferik_company.contact.email
                            : "N/A"}
                        </div>
                        <ChangeEmail
                        ...
                        <div className={"info_span"}>
                          {this.state.teleferik_company.contact.website
                            ? this.state.teleferik_company.contact.website
                            : "N/A"}
                        </div>
                        <ChangeWebsite
                        ...
                        <ChangeLatLng
                          ...
                          object_type={"Teleferik Company"}
                          update_state={this.update_state}
                          lat={this.state.teleferik_company.lat}
                          lng={this.state.teleferik_company.lng}
                        />
                        <div className={"lat_lng_span"}>
                          {this.state.teleferik_company.lat
                            ? this.state.teleferik_company.lat
                            : "N/A"}
                        </div>
                        <div
                        ...
                        >
                          {this.state.teleferik_company.lng
                            ? this.state.teleferik_company.lng
                            : "N/A"}
                        </div>
                        <ContactPersons
                          ...
                          update_state={this.update_state}
                          contact_persons={
                            this.state.teleferik_company.contact_persons
                          }
                        />
                        <div className={"info_descr"}>
                          {this.state.teleferik_company.enabled ? (
                  ...
                  <Grid.Column>
                    <Card>
                      <Card.Header>
                        ...
                        />
                        Map with teleferik company's location
                      </Card.Header>
                      {this.state.teleferik_company.lat ||
                      this.state.teleferik_company.lng ? (
                        <>
                          <Card.Body>
                            <GoogleMap object={this.state.teleferik_company} />
                          </Card.Body>
                        </>
                      ) : (
                        <strong
                        ...
                        >
                          Update latitude / longitude to show teleferik
                      ...
                      <Card.Footer>
                        <small className="mr-auto">
                          ...
                          />
                          Changing teleferik company's lat/lng will also change
                          the map's pin
                        </small>
                      </Card.Footer>
                    </Card>
...
export default TeleferikCompanyOverView;
